package my.dahr.cardiocam.hrometer
...
import my.dahr.cardiocam.hrometer.camera.CameraSupport
import my.dahr.cardiocam.hrometer.camera.CameraSupportImpl
import my.dahr.cardiocam.hrometer.helper.MathHelper
...
open class HeartRateOmeter {
    ...
    protected fun start() {
        ...
        wakelock = powerManager?.newWakeLock(
            ...
            context?.get()?.javaClass?.canonicalName
        )
        wakelock?.acquire(wakeLockTimeOut)

        cameraSupport = CameraSupportImpl().open(0)
        Log.e(tag, "cameraSupport : ${cameraSupport?.parameters}")
    ...
    private fun getScreenDimensions(): Dimension {
        ...
        val display: android.view.Display =
            (context?.get()?.getSystemService(Context.WINDOW_SERVICE) as WindowManager)
                .defaultDisplay
        ...
        try {
            val realSize = Point()
            android.view.Display::class.java.getMethod("getRealSize", Point::class.java)
    ...
    protected fun createCameraPreviewCallback2(): Camera.PreviewCallback {
        ...
        return object : Camera.PreviewCallback {
            ...
            override fun onPreviewFrame(data: ByteArray?, camera: Camera) {
                ...
                val rollingAverage =
                    if (averageArrayCount > 0) averageArrayAverage / averageArrayCount else 0
    ...
    protected fun getSmallestPreviewSize(
        width: Int,
        height: Int,
